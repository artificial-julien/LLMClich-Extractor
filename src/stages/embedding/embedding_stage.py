...
class EmbeddingStage(Stage):
    ...
    def process(self, pipeline_config: PipelineConfig, executions: Iterator[Execution]) -> Iterator[Execution]:
        """
        Process input executions lazily and create new Embedding executions for each item.
        ...
        Yields:
            Embedding executions and unchanged executions
        """
        for base_execution in executions:
            ...
            if base_execution.has_error() or not base_execution.embedding_model_config:
                continue
